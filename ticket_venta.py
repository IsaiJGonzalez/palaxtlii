import sys
from escpos.printer import Win32Raw
import os
from django.conf import settings
from datetime import datetime, date
import locale
import firebase_service as fs
import time  # <--- NECESARIO para la pausa

# Configuración de locale segura
try:
    locale.setlocale(locale.LC_TIME, "es_ES.UTF-8")
except locale.Error:
    try:
        locale.setlocale(locale.LC_TIME, "es_ES")
    except:
        pass

def imprimir_venta(resumen):
    try:
        printer_name = "POS-80"
        p = Win32Raw(printer_name)
        
        # =======================================================
        # CORRECCIÓN DE IMPRESORA (Modo Chino + Acentos)
        # =======================================================
        p._raw(b'\x1c\x2e')       # Desactivar modo Kanji/Chino
        time.sleep(0.1)           # Pausa técnica
        p.charcode('CP850')       # Configurar idioma Español
        # =======================================================

        # Función interna para imprimir el cuerpo del ticket (Cliente o Negocio)
        def imprimir_contenido(titulo_copia):
            # --- LOGO ---
            logo_path = os.path.join(settings.BASE_DIR, "static", "logo_ticket.bmp")
            if os.path.exists(logo_path):
                try:
                    p.set(align='center', bold=False, width=2, height=2)
                    p.image(logo_path)
                except Exception as e:
                    print("⚠️ Error al imprimir logo:", e)

            # --- ENCABEZADO ---
            p.set(align='center', bold=False, width=2, height=2)
            ubicacion = resumen.get('ubicacion', '')
            p.text(f'{ubicacion}\n\n')
            p.text('TICKET VENTA\n')
            p.text(f'{titulo_copia}\n') # Cliente o Negocio
            
            p.set(align='left', bold=False, width=1, height=1)
            
            no_venta = resumen.get('no_venta', '')
            fecha_str = resumen.get('fecha_venta', '')
            try:
                fechaOb = datetime.strptime(fecha_str, "%Y-%m-%d")
                fecha_f = fechaOb.strftime('%d de %B de %Y')
            except:
                fecha_f = fecha_str

            atendio_nom = resumen.get('at_nom_emp', '')
            atendio_num = resumen.get('at_no_emp', '')
            
            p.text(f'No. Venta: {no_venta}\n')
            p.text(f'Fecha:     {fecha_f}\n')
            p.text(f'Atendió:   {atendio_num} - {atendio_nom}\n')
            p.text('-' * 48 + '\n')

            # --- PRODUCTOS ---
            p.set(bold=True)
            p.text(f"{'Producto':<24}{'Cant':>6}{'P.U.':>8}{'Importe':>10}\n")
            p.set(bold=False)

            loc_emp = resumen.get('loc_emp')
            productos = resumen.get('productos', [])
            
            for producto in productos:
                cantidad = producto.get('cantidad', 0)
                producto_id = producto.get('productoId')
                nombre = ""
                
                # Obtener nombre según ubicación
                if loc_emp == 1:
                    nombre = fs.obtener_producto_vh(producto_id)
                elif loc_emp == 2:
                    nombre = fs.obtener_producto_mc(producto_id)
                
                if not nombre: nombre = "Producto"

                precio = float(producto.get('precio_unitario', 0.0))
                importe = float(producto.get('subtotal', 0.0))

                # Dividir nombre largo
                lineas_nombre = [nombre[i:i+24] for i in range(0, len(nombre), 24)]
                for i, linea in enumerate(lineas_nombre):
                    if i == len(lineas_nombre) - 1:
                        p.text(f'{linea:<24}{cantidad:>6}{precio:>8.2f}{importe:>10.2f}\n')
                    else:
                        p.text(f"{linea:<24}\n")

            p.text('-' * 48 + '\n')

            # --- TOTALES ---
            total = float(resumen.get('total', 0.0))
            metodo_p = resumen.get('metodo_pago', '')
            mix_ef = float(resumen.get('mix_ef', 0.0))
            mix_tar = float(resumen.get('mix_tar', 0.0))
            recibido = float(resumen.get('recibido', 0.0))
            cambio = float(resumen.get('cambio', 0.0))
            no_operacion = resumen.get('no_operacion', '')

            p.set(bold=False)
            p.text(f'Método de Pago: {metodo_p.upper()}\n\n')
            
            # Total en grande
            p.set(bold=True)
            p.text(f"{'Total:':<20}${total:>8.2f}\n")
            p.set(bold=False)

            if metodo_p == 'mixto':
                p.text(f"{'Tarjeta:':<20}${mix_tar:>8.2f}\n")
                if no_operacion:
                    p.text(f'No. Operación: {no_operacion}\n')
                p.text(f"{'Efectivo:':<20}${mix_ef:>8.2f}\n")
                p.text(f"{'Recibido:':<20}${recibido:>8.2f}\n")
                p.text(f"{'Cambio:':<20}${cambio:>8.2f}\n\n")
            
            elif metodo_p == 'efectivo':
                p.text(f"{'Recibido:':<20}${recibido:>8.2f}\n")
                p.text(f"{'Cambio:':<20}${cambio:>8.2f}\n\n")
            
            elif metodo_p in ['tarjeta', 'transferencia']:
                if no_operacion:
                    p.text(f'Número de Operación: {no_operacion}\n\n')

            # --- PIE DE PÁGINA ---
            p.set(align='center')
            if titulo_copia == "Cliente":
                p.text("¡Gracias por su preferencia!\n\n")
                p.text('***Reserva tu pastel perfecto***\n\n')
                p.text("Whatsapp: 427 159 0622\n")
            
            # Fecha impresión
            fecha_hora = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
            p.text(f"\nImpreso: {fecha_hora}\n")
            p.text("\n") # Espacio extra antes del corte

        # 1. IMPRIMIR COPIA CLIENTE
        imprimir_contenido("Cliente")
        p.cut()

        # 2. IMPRIMIR COPIA NEGOCIO (Descomentado y funcional)
        # Si no quieres copia negocio, comenta las siguientes 2 líneas:
        imprimir_contenido("Negocio")
        p.cut()

        p.close()

    except Exception as e:
        print("❌ Error general al imprimir ticket venta:", e)